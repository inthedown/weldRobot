		...
		<template v-for="(step, index) in steps">
			<view class="step-item" :key="'step-' + index">
				<view class="step-content" @click="handleStepClick(index, step)">
					<uni-icons :type="step.icon" :color="getColor(index)" size="24" class="uni-icon" />
					<text class="step-text" :style="{ color: getColor(index) }">{{ step.name }}</text>
				</view>
			</view>
			<!-- 仅在不是最后一个时渲染箭头 -->
			<view v-if="index < steps.length - 1" class="arrow-wrapper" :key="'arrow-' + index">
				<uni-icons type="arrow-right" size="20" color="#ccc" class="arrow-icon" />
			</view>
		</template>
	</view>
	...
	export default {
		...
		methods: {
			...
			handleStepClick(index, step) {
				console.log(this.localWeldClass==='');
